...
def find_version():
    ...
    try:
        ...
        git_version = (
            git_output.replace("dirty", "mod").replace("-", "+", 1).replace("-", ".")
        )
...
def locate_ob():
    ...
    if in_conda:
        ...
        if os.path.isdir(include_dirs):
            ...
            print(
                "Warning: We are in an Anaconda env, but Openbabel is not installed here."
            )
    ...
    if output:
        ...
        if not ob_ver.version[:2] == py_ver.version[:2]:
            print(
                "Warning: Open Babel {}.{}.x is required. Your version ({}) may not be compatible.".format(
                    py_major_ver, py_minor_ver, ob_ver
                )
            )
        include_dirs = execute_command(
            "pkg-config --variable=pkgincludedir %s" % pcfile
        )[0].strip()
        library_dirs = execute_command("pkg-config --variable=libdir %s" % pcfile)[
            0
        ].strip()
...
class CustomBuildExt(build_ext):
    ...
    def finalize_options(self):
        ...
        if self.boost_include_dir is None and self.boost_library_dir is None:
            ...
            print(
                "Boost library location was automatically guessed at %s."
                % self.boost_include_dir
            )
    ...
    def swig_sources(self, sources, extension):
        try:
            ...
            print(
                ...
                "For example:",
                "  python setup.py build_ext -I{} -L{}".format(
                    self.include_dirs, self.library_dir
                ),
    ...
    def build_extensions(self):
        ...
        include_system = set(self.include_dirs).intersection(
        ...
        for remove_flag in remove_flags:
            try:
                ...
                print(
                    "Warning: compiler flag %s is not present, cannot remove it."
                    % remove_flag
                )
                pass
